...
class CommunityDetection(Qubo):
    def __init__(self, adjacency_matrix, number_communities, gamma, beta):
        super().__init__()
        self.adjacency_matrix = np.asarray(adjacency_matrix)
        self.number_communities = number_communities
        self.gamma = (
            gamma  # parameter defining how soft the one hot encoding terms is
        )
    ...
    def modularity_matrix(
        self,
    ):  # note modularity can break if adjacency has negative weights or zero total edge mass
    ...
    def decode_solution(self, x):
        ...
        for i in range(n):
            vals = np.array(
                [x[c * n + i] for c in range(self.number_communities)]
            )
